...
class FCN32VGG:
    ...
    def __init__(self, vgg16_npy_path=None):   # 构造函数
        if vgg16_npy_path is None:
            # sys.modules存储着加载的模块；
            # self表示类的实例，而self.__class__则指向类，
            # self.__class__.__module__指向所在的module,即fcn32_vgg.py
            #__file__文件的绝对路径
            path = sys.modules[self.__class__.__module__].__file__
            # print path  os.pardir 获取当前目录的父目录字符串名
            path = os.path.abspath(os.path.join(path, os.pardir))
            # print path  os.path.join()目录相加
        ...
        # Weights数据在data_dict[name][0]里，bias数据在data_dict[name][1]里
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        if random_init_fc8:
            ...
            self.score_fr = self._fc_layer(self.fc7, "score_fr",
                                           ...
                                           relu=False)
        # 找出20个feature map 里对应每一个元素最大值的索引（结果是一个矩阵）
    ...
    def _conv_layer(self, bottom, name):
        with tf.variable_scope(name) as scope:
            filt = self.get_conv_filter(name)  # 获得预训练数据里的Weights
            # conv2d计算2D卷积
            conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')

            conv_biases = self.get_bias(name) # 获得预训练数据里的bias
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            if name == 'fc6':
                # 所得到的filt即为从data_dict得到的参数reshape成卷积核后的参数
            ...
            # fc 将原来的全连接层改为卷积层，fc6卷积核大小为7*7，后来的为1*1
            conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')
